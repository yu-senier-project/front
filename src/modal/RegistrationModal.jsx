...
function RegistrationModal({ open, handleClose }) {
    ...
    const sendAuthCode = () => {
        ...
        axios
            .get(BaseUrl + '/api/v1/email-auth/request/' + formData.email + '@' + companyEmail)
            .then((res) => {
                console.log(res);
                setIsSend(true);
            })
    ...
    const handleCompanyName = useCallback(
        debounce((e) => {
            ...
            if (value.trim()) {
                axios
                    .get(`http://43.203.69.159:80/api/v1/company/search?keyword=${value}`)
                    .then((response) => {
    ...
    const getCompanyEmail = (companyName) => {
        axios
            .get(`http://43.203.69.159:80/api/v1/company/${companyName}/email`)
            .then((response) => {
        ...
        checkAuthCode().then((response) => {
            if (response.statusCode === 200) {
                const data = {
                    ...
                    birth: `${date.year}-${String(date.month).padStart(2, '0')}-${String(date.day).padStart(2, '0')}`,
                };
                console.log(data);
            } else {
                alert('실패');
                        ...
                        {companys.length > 0 ? (
                            <ul>
                                {companys.map((company, index) => (
                                    <li key={index}>
                                        {company.name}
                                        <button onClick={() => getCompanyEmail(company.name)}>선택</button>
                                    </li>
                                ))}
                            </ul>
        ...
        case 1:
            content = (
                <div className="register-modal">
                    ...
                    <Input size="Large" placeholder={'직무'} onChange={handleInputChange} name={'position'} />
            ...
            content = (
                <div className="register-modal">
                    ...
                    <div className="id-space">
                        ...
                        />
                        <button
                            className="duplication_button"
                            onClick={() => {
                                axios
                                    .get(`http://43.203.69.159/api/v1/auth/register/check/${formData.userNickName}`)
                                    .then((res) => {
                                        console.log(res);
